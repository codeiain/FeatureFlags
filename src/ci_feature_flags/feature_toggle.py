...
class Toggle:
    ...
    def __bool__(self):
        if (
            self.creation_date
            and (date.today() - self.creation_date).days > self.max_lifetime
        ):
            logger.warning(
...
class TogglesList:
    def __init__(self, document):
        ...
        if not_configured:
            raise Exception(
                f"The following toggles are not configured: {not_configured}"
            )
    ...
    def __getattribute__(self, attr):
        ...
        if not attr.startswith("_") and attr in self._toggle_config:
            logger.info(
                f"Checking toggle {attr} in {calframe[1][1]}:{calframe[1][2]} {calframe[1][3]}()"
            )
